...
member_counts_all = df.groupby(['ab_test_group', 'is_member']).first_name.count().reset_index()
final_member_pivot = member_counts_all.pivot(columns = 'is_member', index = 'ab_test_group', values = 'first_name').reset_index()
...
plt.title('Visitors Who Apply To MuscleHub')
...
#Percent Applications Who Purchase
ax = plt.subplot()
...
plt.title('Applicants Who Purchase Membership To MuscleHub')
plt.savefig('percent_applicants_join.png')
plt.show()
...
#Percent Visitors Who Purchase
ax = plt.subplot()
plt.bar(range(len(final_member_pivot)), final_member_pivot['Percent Purchase'].values)
ax.set_xticks(range(len(app_pivot)))
ax.set_xticklabels(['Fitness Test', 'No Fitness Test'])
ax.set_yticks([0, .02, .04, .06, .08, .1, .12, .14])
ax.set_yticklabels(['0%', '2%', '4%', '6%', '8%', '10%', '12%', '14%'])
plt.title('Visitors Who Purchase Membership To MuscleHub')
plt.savefig('percent_visitors_join.png')
plt.show()
